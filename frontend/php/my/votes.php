...
if (user_isloggedin())
{
  ...
  if ($submit)
    {
      $result = db_execute("SELECT vote_id,tracker,item_id FROM user_votes "
                           ."WHERE user_id = ? ORDER BY howmuch DESC, item_id "
                           ."ASC LIMIT 100", array(user_getid()));
      unset($count);
  ...
  print '<p>'._("Here is the list of your votes.").' '
        .sprintf(ngettext("%s vote remains at your disposal.",
                          "%s votes remain at your disposal.", $remaining_votes),
                 $remaining_votes).'</p>';
      ...
      while($row = db_fetch_array($result))
	{
	  ...
	  print '<div class="'
	    ...
	    .'&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.$GLOBALS['sys_home']
            .$row['tracker'].'/?func=detailitem&amp;item_id='.$row['item_id'].'">'
	    .'<img src="'.$GLOBALS['sys_home'].'images/'.SV_THEME
            .'.theme/contexts/'.$icon.'.png" class="icon" alt="'
            .$row['tracker'].'" />'
	    .htmlentities(db_result($res_item, 0, 'summary')).', '
            .sprintf(ngettext("%s vote", "%s votes", db_result($res_item, 0, 'vote')),
                     db_result($res_item, 0, 'vote'))
...
#  ################################ Submit
      ...
      print '<br /><div align="center" class="noprint">'
            .'<input type="submit" name="submit" class="bold" value="'
            ._("Submit Changes").'" /></div></form>';

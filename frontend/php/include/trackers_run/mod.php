  ...
  if (member_check(0,$group_id,member_create_tracker_flag(ARTIFACT).'2', 1))
    {
      print '<p>'.help(_("You are both technician and manager for this tracker."),
        array(_("technician")
              ...
              _("manager")=>_("fully manage the items")))."</p>\n";
...
# ################################ Start the form
  ...
  .utils_user_link(user_getname(db_result($result,0,'submitted_by')),
                   user_getrealname(db_result($result,0,'submitted_by'))).'</td>
          <td colspan="'
  .($fields_per_line)
      ...
      if ($sz > $max_size)
	{
	  ...
	  $j++;
      ...
      else
        {
          ...
          print '<td valign="middle"'.$field_class.' width="15%">'.$label.$star
            .'</td><td valign="middle"'.$field_class.' width="35%">'.$value
            ."</td>\n";
          $i++;
...
# "search an item of [The Bug Tracker Only] of [Any Project]".
  $group_select .= '<option value="any"'.$selected.'>'._("Any Project")
                   ."</option>\n";

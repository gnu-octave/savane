...
# Remove extra comma
			...
			.'<td align="center">--------</td>';
                  ...
                  print '<td align="center">'.utils_link(htmlentities ($_SERVER['PHP_SELF']).'?group='.$group.'&amp;func=deltransition&amp;transition_id='.$transition['transition_id'].'&amp;list_value=1&amp;field='.$field, '<img src="'.$GLOBALS['sys_home'].'images/'.SV_THEME.'.theme/misc/trash.png" border="0" alt="'._("Delete this transition?").'" />').'</td>';
                  print '</tr>';
                }
...
# other places
		...
		'<td><a href="'.htmlentities ($_SERVER['PHP_SELF']).'?update_canned=1&amp;item_canned_id='.
		db_result($result, $i, 'bug_canned_id').'&amp;group_id='.$group_id.'">'.
		db_result($result, $i, 'title').'</A></TD>'.
		'<td>'.substr(db_result($result, $i, 'body'),0,360).'...'.
		'<td>'.db_result($result, $i, 'order_id').'<td class="center"><a href="'.htmlentities ($_SERVER['PHP_SELF']).'?delete_canned=1&amp;item_canned_id='.
      ...
      while ( $field_name = trackers_list_all_fields() )
	{
	  ...
	  if ( trackers_data_is_select_box($field_name)
	       ...
	       && trackers_data_is_used($field_name) )
	    {
	      ...
	      print '<tr class="'. utils_get_alt_row_color($i) .'">'.
		'<td><a href="'.htmlentities ($_SERVER['PHP_SELF']).'?group_id='.$group_id.'&list_value=1&field='.$field_name.'">'.trackers_data_get_label($field_name).'</a></td>'.
...
# Now the special canned response field
      print '<tr class="'. utils_get_alt_row_color($i) .'">';
      print "<td><a href=\"";
      print htmlentities ($_SERVER['PHP_SELF'])."?group_id=$group_id&amp;create_canned=1\">"._("Canned Responses").'</a></td>';

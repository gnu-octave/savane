...
$res_export = db_execute("SELECT * FROM trackers_export WHERE user_name=?
                          AND unix_group_name=? AND status<>'I'
...
if ($update)
  {
    ...
    if ($create)
        ...
        while (list($field,$value_id) = each($url_params))
          {
            ...
            if (trackers_data_is_select_box($field)
                && !trackers_isvarany($url_params[$field]))
              {
                $where .= ' AND '.$field.' IN ('
                       .implode(',',
                                array_fill(0, count($url_params[$field]), '?'))
                       .') ';
            ...
            elseif (trackers_data_is_date_field($field)
                     && $url_params[$field][0])
              {
                ...
                preg_match("/\s*(\d+)-(\d+)-(\d+)/", $url_params[$field][0],
                           $match);
      ...
      # It s a text field accept. Process INT or TEXT,VARCHAR fields differently
                    list($expr, $params) =
                      trackers_build_match_expression($field,
                                                      $url_params[$field][0]);
                    $where .= ' AND $expr ';
            ...
            if ($details_search == 1 && $summary_search == 1)
              {
                ...
                $where .= '( ( ';
                list($expr, $params) =
                  trackers_build_match_expression('details',
                                                  $url_params['details'][0]);
                $where .= $expr;
                $where_params = array_merge($where_params, $params);
                $where .= ' ) OR ( ';
                list($expr, $params) =
                  trackers_build_match_expression('summary',
          ...
          case 'frequent':
            ...
            if ($date_frequent_day < 7)
            ...
            else
              {
                ...
                if ($timestamp < time())
                  $timestamp = mktime($hour, 0, 0, $current_month,
                                      $current_day+1);
              }
          }
        ...
        session_redirect($GLOBALS['sys_home']
                         ."task/export-createtask.php?group="
                         .rawurlencode($group)."&export_id=$insert_id&from="
                         .ARTIFACT);
      } /* if ($create) */
    ...
    if ($delete)
      {
        ...
        $task_id = db_result(db_execute("SELECT task_id FROM trackers_export
                                         WHERE export_id=? LIMIT 1",
                                        array($export_id)), 0, 'task_id');
        // Delete the entry
        $result = db_execute("DELETE FROM trackers_export WHERE export_id=?
                              AND user_name=? LIMIT 1",
...
# TRANSLATORS: the argument is job id (a number).
            ...
            session_redirect($GLOBALS['sys_home']
                             ."task/export-updatetask.php?group="
                             .rawurlencode($group)
                             ."&export_id=$export_id&task_id=$task_id&from="
                             .ARTIFACT);
          }
        else
          {
# TRANSLATORS: the argument is job id (a number).
        ...
        $res_export = db_execute("SELECT * FROM trackers_export
                                  WHERE user_name=? AND unix_group_name=?
                                  AND status<>'I' ORDER BY export_id ASC",
                                 array(user_getname(), $group));
        $export_count = db_numrows($res_export);
      } /* if ($delete) */
 } # if ($update)
    ...
    for ($i = 0; $i < $export_count; $i++)
      {
       ...
       print utils_link(htmlentities ($_SERVER['PHP_SELF']).'?update=1&amp;delete='
                        .db_result($res_export, $i, 'export_id').'&amp;group='
                        .$group,
       ...
       $export_url = $GLOBALS['sys_https_url'].$GLOBALS['sys_home']
                     ."export/$group/".user_getname()."/"
                     .db_result($res_export, $i, 'export_id').".xml";
       print '<br />'.sprintf(_("URL: %s"),
                              utils_link($export_url, $export_url));
...
# beginning of a new row
        ...
        $labels .= '<td>'.trackers_field_label_display($field,
                                                       $group_id,
                                                       false,false).'</td>';
        ...
        if (trackers_data_is_select_box($field))
          {
            ...
            $boxes .=
              trackers_field_display($field,$group_id,$value,false,false,
                                     ($printer?true:false),false,true,'None',
                                     true,'Any');
          }
        elseif (trackers_data_is_date_field($field))
          {
            if ($advsrch)
              ...
              {
                $boxes .= trackers_field_date_operator($field,$is_multiple,
                                                       $printer)
...
# Make sure the last few cells are in the table
    ...
    # Time selection
    print '<br />';
    print '<span class="preinput">'._("The export should be generated:")
          .'</span><br />';

    # ASAP case
    print '&nbsp;&nbsp;&nbsp;'
          .form_input("radio", "date_mainchoice", "asap\" checked=\"checked")
          .' '._("as soon as possible").'<br />';

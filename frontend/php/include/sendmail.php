          ...
          if (user_exists($touid, true))
            {
              ...
              if (is_array($exclude) && array_key_exists(user_getname($touid),
                                                         $exclude))
                continue;

              # If we get here, we have a squad and we will store all the
              # squad members uid.
              $result_squad = db_execute("SELECT user_id FROM user_squad
                                          WHERE squad_id=?",
                                         array($touid));
          ...
          if (user_exists($touid))
            {
              ...
              if (user_get_preference("subject_line", $touid) != "")
                {
                  $list[$i] = $v;
                  $user_subject[$v] = sendmail_format_subject_line(
...
# TRANSLATORS: the argument is user's name.
  print $HTML->box_top(sprintf(_("Send a Message to %s"),
                               user_getrealname($user_id)));
  print '<p class="warn">'.("If you are writing for help, did you read the
...
function sendmail_create_msgid ()
{
  ...
  return date("Ymd-His", time()).".sv".user_getid().".".mt_rand(0,100000)."@"
         .$_SERVER["HTTP_HOST"];
}
?>

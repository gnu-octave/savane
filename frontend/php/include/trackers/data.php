...
function &trackers_data_get_notification_settings($group_id, $tracker_name)
{
  ...
  $settings['private_exclude'] = db_result($result,0,
                                           $tracker_name
...
function trackers_data_show_notification_settings($group_id, $tracker_name,
                                                  $show_intro_msg)
{
...
.'</span><br />&nbsp;&nbsp;<INPUT TYPE="TEXT" NAME="'.$tracker_name.'_cat_'.$i
...
.'</span><br />&nbsp;&nbsp;<INPUT TYPE="TEXT" NAME="'.$tracker_name
...
{
  global $BF_USAGE_BY_ID,$BF_USAGE_BY_NAME;
  return($by_field_id ? $BF_USAGE_BY_ID[$field]['custom']
                        : $BF_USAGE_BY_NAME[$field]['custom']);
}
...
function trackers_data_mandatory_flag ($field, $by_field_id=false)
{
  ...
  if ($by_field_id)
  ...
  else
    {
      ...
      elseif (isset($BF_USAGE_BY_NAME[$field]['empty_ok']))
...
{
  global $BF_USAGE_BY_ID,$BF_USAGE_BY_NAME;
  return($by_field_id ? $BF_USAGE_BY_ID[$field]['group_id']
                        : $BF_USAGE_BY_NAME[$field]['group_id']);
}
...
{
  global $BF_USAGE_BY_ID,$BF_USAGE_BY_NAME;
  return($by_field_id ? $BF_USAGE_BY_ID[$field]['col_width']
                        : $BF_USAGE_BY_NAME[$field]['col_width']);
}
...
function trackers_data_get_display_size($field, $by_field_id=false)
{
  ...
  if ($by_field_id)
  ...
  else
    {
      ...
      elseif (isset($BF_USAGE_BY_NAME[$field]['display_size']))
  ...
  if ($res = trackers_data_is_default_value($item_fv_id))
    {
      ...
      $where_cond_params = array($arr['bug_field_id'], $arr['value_id'],
                                 $group_id);
    }
  else
    {
  ...
  if ($spamscore < 5)
    {
      ...
      spam_add_to_spamcheck_queue($item_id, db_insertid($result), $artifact,
                                  $group_id, $spamscore);
    }
  return $result;
}
  ...
  while (list($field,$value) = each($vfl))
    {
      ...
      if (array_key_exists($field_id, $field_transition)
        ...
        {
          if (array_key_exists("any", $field_transition[$field_id])
              && array_key_exists($value,
                                  $field_transition[$field_id]["any"]))
            {
              $field_transition_cc =
              ...
              if (!isset($changes[$field])
                ...
                $field_transition_id =
                  $field_transition[$field_id]["any"][$value]['transition_id'];
            }
          else if (array_key_exists($old_value, $field_transition[$field_id])
                   && array_key_exists($value,
          ...
          if ($res3 && db_numrows($res3) > 0)
            {
              ...
              if ($details)
                $details .= "\n\n";
              $details .= utils_unconvert_htmlspecialchars(db_result($res3, 0,
                                                                     'body'));
...
original item report information."), $row_attachment['filename']), 1);
      ...
      # Duplicate CC List.
      $res_cc = db_execute("SELECT * FROM ".ARTIFACT."_cc WHERE bug_id=?",
                           array($item_id));
...
original item report information."), $row_cc['email']), 1);
      ...
      list($additional_address, $sendall) =
        trackers_data_get_item_notification_info($new_item_id,
                                                 $reassign_change_artifact, 1);
      trackers_mail_followup($new_item_id, $additional_address, false, false,
                             $reassign_change_artifact);
...
function trackers_data_create_item($group_id,$vfl,&$extra_addresses)
{
  ...
  while (list($field,$value) = each($vfl))
    {
      ...
      if (array_key_exists($field_id, $field_transition)
        ...
        {
           if (array_key_exists("any", $field_transition[$field_id])
               && array_key_exists($value, $field_transition[$field_id]["any"]))
             {
               $field_transition_cc =
                 $field_transition[$field_id]["any"][$value]['notification_list'];
           ...
           else if (array_key_exists("100", $field_transition[$field_id])
                    && array_key_exists($value,
                                        $field_transition[$field_id]["100"]))
             {
               $field_transition_cc =
                 $field_transition[$field_id]["100"][$value]['notification_list'];
               # Register the transition, but only if the field it is about
               # was not filled in the form
               if (!is_array($changes[$field]) ||
                   (!array_key_exists('del', $changes[$field]) &&
                    !array_key_exists('add', $changes[$field])))
                 $field_transition_id =
                   $field_transition[$field_id]["100"][$value]['transition_id'];
             }
        }
  ...
  if (!$item_id)
    {
      fb(_("New item insertion failed, please report this issue to the
administrator"), 1);
      return false;
    }
...
{
  return db_execute("SELECT role_id,event_id,notify FROM trackers_notification
                     WHERE user_id=?",
                    array($user_id));
}
...
{
  return db_execute("DELETE FROM trackers_notification WHERE user_id=?",
                    array($user_id));
}

function trackers_data_insert_notification($user_id, $arr_roles, $arr_events,
                                           $arr_notification)
{
...
{
  return db_execute("SELECT user_id,group_id FROM trackers_watcher
                     WHERE watchee_id=?", array($user_id));
}

function trackers_data_get_watchees($user_id)
{
  return db_execute("SELECT watchee_id,group_id FROM trackers_watcher
...
{
  return db_execute("DELETE FROM trackers_watcher WHERE user_id=?
                     AND watchee_id=? AND group_id=?",

...
';
  ...
  print specific_showinput(no_i18n("Name:"), '<input type="text" name="name" value="'
.$row_grp['name'].'" size="'.$textfield_size.'" />');
  print specific_showinput(no_i18n("Base Host:"), '<input type="text" name="base_host" value="'
                           .$row_grp['base_host'].'" size="'.$textfield_size.'" />');
  print specific_showinput(no_i18n("Description (will be added on each project main page):"),
                           '<textarea cols="'.$textfield_size
                           .'" rows="6"wrap="virtual" name="description">'
                           .$row_grp['description'].'</textarea>');
  print specific_showinput(no_i18n("Admin Email Address:"),
                           '<input type="text" name="admin_email_adress" value="'
...
';
  ...
  print '<br />
'.specific_showinput(no_i18n("Selected SCM:"), '<select name="homepage_scm">
...
no_i18n("Homepage Dir (path on the filesystem) [BACKEND SPECIFIC]:"),
 '<input type="text" name="dir_homepage" value="'.$row_grp['dir_homepage']
 .'" size="'.$textfield_size.'" />');
  print specific_showinput(no_i18n("Homepage URL:"),
 '<input type="text" name="url_homepage" value="'.$row_grp['url_homepage']
 .'" size="'.$textfield_size.'" />');
  print specific_showinput(no_i18n("Homepage CVS view URL (webcvs, viewcvs):"),
 '<input type="text" name="url_cvs_viewcvs_homepage" value="'
...
# TRANSLATORS: the argument is VCS name (like Subversion).
 ...
 '<input type="checkbox" name="can_use_'.$vcs_offix.'" value="1"'
...
';
 ...
 '<input type="checkbox" name="can_use_download" value="1"'
 .(($row_grp['can_use_download']==1) ? ' checked="checked"' : '').' />');
  html_select_typedir_box("dir_type_download",
			  $row_grp['dir_type_download']);
  print specific_showinput(
no_i18n("Repository Dir (path on the filesystem) [BACKEND SPECIFIC]:"),
  '<input type="text" name="dir_download" value="'.$row_grp['dir_download']
  .'" size="'.$textfield_size.'" />');
  print specific_showinput(no_i18n("Repository URL:"),
 '<input type="text" name="url_download" value="'.$row_grp['url_download']
 .'" size="'.$textfield_size.'" />');
...
accepted licenses.').'</p>';
 ...
 '<input type="checkbox" name="can_use_license" value="1"'
...
development status.').'</p>';
  ...
  '<input type="checkbox" name="can_use_devel_status" value="1"'
...
';
 ...
 '<input type="checkbox" name="can_use_mailing_list" value="1"'
...
';
 ...
 '<input type="text" name="mailing_list_host" value="'
...
no_i18n("Mailing list address (would be %LIST@gnu.org for GNU projects at sv.gnu.org):"),
 '<input type="text" name="mailing_list_address" value="'
...
sv.gnu.org) [BACKEND SPECIFIC]:"),
 '<input type="text" name="mailing_list_virtual_host" value="'
...
<p class="warn">'.no_i18n('Do no add any @hostname here!').'</p>';
  print specific_showinput(no_i18n("Mailing list name format:"),
   '<input type="text" name="mailing_list_format" value="'
   .$row_grp['mailing_list_format'].'" size="'.$textfield_size.'" />');
  print specific_showinput(no_i18n("Listinfo URL:"),
   '<input type="text" name="url_mailing_list_listinfo" value="'
...
'<input type="text" name="url_mailing_list_subscribe" value="'
...
"majordomo_interface.php?func=unsubscribe&amp;list=%LIST&amp;mailserver=listbox.server@cern.ch):"),
 ...
 .$row_grp['url_mailing_list_archives_private'].'" size="'.$textfield_size.'" />');
  ...
  '<input type="text" name="url_mailing_list_admin" value="'
  ...
  print $HTML->box_top(no_i18n("Forum"));
  print '<p>'.no_i18n('Forum is a deprecated feature of Savane. We do not recommend using
it and we do not maintain this code any longer.').'</p>';
  print specific_showinput_inverted(no_i18n("Can use forum"),
  '<input type="checkbox" name="can_use_forum" value="1"'
  .(($row_grp['can_use_forum']==1) ? ' checked="checked"' : '').' />');
  ...
  print $HTML->box_top(no_i18n("Support Request Manager"));
  print '<p>'.no_i18n('This is one of the main issue tracker of Savane. Project are
supposed to use it as primary interface with end user.').'</p>';
  print specific_showinput_inverted(no_i18n("Can use support request tracker"),
  '<input type="checkbox" name="can_use_support" value="1"'
  .(($row_grp['can_use_support']==1) ? ' checked="checked"' : '').' />');
  ...
  print $HTML->box_top(no_i18n("Bug Tracker"));
  print '<p>'.no_i18n('This is one of the main issue tracker of Savane. Unlike the
...
suggestion.').'</p>';
  print specific_showinput_inverted(no_i18n("Can use bug tracker"),
  '<input type="checkbox" name="can_use_bug" value="1"'
  .(($row_grp['can_use_bug']==1) ? ' checked="checked"' : '').' />');
  ...
  print $HTML->box_top(no_i18n("Task Manager"));
  print '<p>'.no_i18n('This is one of the main issue tracker of Savane. Unlike the
support tracker, it is supposed to be used mainly to organize the workflow
amongs project members related to planned tasks. It\'s the counterpart of the
bug tracker for regular and planned activities.').'</p>';
  print specific_showinput_inverted(no_i18n("Can use task manager"),
 '<input type="checkbox" name="can_use_task" value="1"'
 .(($row_grp['can_use_task']==1) ? ' checked="checked"' : '').' />');
  ...
  print $HTML->box_top(no_i18n("Patch Manager"));
  print '<p>'.no_i18n('This is a deprecated issue tracker. It was originally designed
...
defined out of the box.').'</p>';
  print specific_showinput_inverted(no_i18n("Can use patch manager (deprecated)"),
 '<input type="checkbox" name="can_use_patch" value="1"'.
 (($row_grp['can_use_patch']==1) ? ' checked="checked"' : '').' />');
  ...
  print $HTML->box_top(no_i18n("News Manager"));
  print specific_showinput_inverted(no_i18n("Can use news manager"),
  '<input type="checkbox" name="can_use_news" value="1"'
  .(($row_grp['can_use_news']==1) ? ' checked="checked"' : '').' />');
